...
### Character encoding resources
  ...
  * I've translated this blog post [Guide to fixing encoding problems in Ruby][encoding-probs-ruby] into R as the first step to developing a lesson: [Encoding in R](#character-encoding)
...
You can collapse a character vector of length `n > 1` to a single string with `str_c()`, which also has other uses (see [next section](#catenate-vectors)).
