...
* [Stack Exchange outage](https://stackstatus.net/post/147710624694/outage-postmortem-july-20-2016)
* [Regexes to validate/match email addresses](https://emailregex.com)
* [Fixing an Atom bug](https://davidvgalbraith.com/how-i-fixed-atom/)
...
* The [Strings chapter] of [R for Data Science] [@wickham2016] is a great resource.
...
* Chapter \@ref(character-encoding) - I've translated this blog post, ["3 Steps to Fix Encoding Problems in Ruby"], into R as the first step to developing a lesson.
...
You can collapse a character vector of length `n > 1` to a single string with `str_c()`, which also has other uses (see the [following section](#catenate-vectors)).
...
The country names in the `gapminder` data frame are convenient for examples. Load it now and store the `r nlevels(gapminder::gapminder$country)` unique country names to the object `countries`.
...
* The [Strings chapter] of [R for Data Science] [@wickham2016].

<!--Links-->
[Strings chapter]: https://r4ds.had.co.nz/strings.html
