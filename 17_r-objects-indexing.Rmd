...
<!--[Link to original content](http://stat545.com/block004_basic-r-objects.html) -->
...
Most common, useful ways to index a vector
  ...
  * __vector of negative integers: specifying the losers
...
> Sidebar: What is `I()`, used when creating the variable $y$ in the above data.frame? Short version: it tells R to do something _quite literally_. Here we are protecting the letters from being coerced to factor. We are ensuring we get a character vector. Note we let character-to-factor conversion happen in creating the $v$ variable above. More about (foiling) R's determination to convert character data to factor can be found [here](#factors).

<!--Original stat545 lesson links to an older lesson from 2013 [here][https://www.stat.ubc.ca/~jenny/STAT545A/block08_bossYourFactors.html]-->
