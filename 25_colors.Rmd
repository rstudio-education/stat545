...
```{r decimal-hexadecimal, echo = FALSE}
library(knitr)
foo <- t(cbind(hex = I(c(as.character(0:9), LETTERS[1:6])),
               decimal = I(as.character(0:15))))
colnames(foo) <- seq(1:16)

kable(foo, booktabs = TRUE, caption = "Hexademical (base 16) to Decimal (base 10)")
```
...
```{r hex-codes-rgb, echo = FALSE}
foo <- data.frame(color_name = c("blue", "green", "red"),
                  hex = c("#0000FF", "#00FF00", "#FF0000"),
                  red = c(0, 0, 255),
                  green = c(0, 255, 0),
                  blue = c(255, 0, 0))
kable(foo, booktabs = TRUE, caption = "Hex color codes for RGB colors")
```

Here are shades of gray:

```{r hex-codes-gray, echo = FALSE}
...
foo <- data.frame(color_name = c("white, gray100", "gray67",
                  ...
                  green = j_intensity,
                  blue = j_intensity)
kable(foo, booktabs = TRUE, caption = "Hex color codes for shades of gray")
```
