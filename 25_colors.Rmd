...
<!--[Link to original content](http://stat545.com/block018_colors.html)-->

<!--The link below is broken. Found it listed [here](https://www.showmeshiny.com/r-graph-catalog/) which links this [repo](https://github.com/jennybc/r-graph-catalog) as the source code
...
<!--This link is broken, replace with something similar?
...
I encourage you to install `viridis` and read [the vignette][viridis-vignette]. It is easy to use these palettes in ggplot2 via `scale_color_viridis()` and `scale_fill_viridis()`. Taking control of color palettes in ggplot is covered elsewhere: [Taking control of qualitative colors in `ggplot2`](#qualitative-colors).

Here's are two examples that show the viridis palettes.
...
![Image Source: [sjmgarnier][viridis-example-plots]](img/viridis-sample2.png)
...
<!--Link below auto-downloads a pdf file, this was the best link I could find. Do we want to change this?-->
  * "Why Should Engineers and Scientists Be Worried About Color?"[(pdf)][worry-about-color]
...
* Achim Zeileis, Kurt Hornik, Paul Murrell (2009). Escaping RGBland: Selecting Colors for Statistical Graphics. Computational Statistics & Data Analysis, 53(9), 3259-3270. [DOI][escaping-rgbland-doi] | [PDF][escaping-rgbland-pdf]
...
* Earl F. Glynn (Stowers Institute for Medical Research)
  ...
  + informative talk "Using Color in R"[(pdf)][stowers-using-color-in-R],  though features some questionable *use* of color itself
* Blog post [My favorite RGB color][favorite-rbg-color] on the Many World Theory blog
* ggplot2: Elegant Graphics for Data Analysis [available via SpringerLink][elegant-graphics-springer] by Hadley Wickham, Springer (2009)
...
* "Why Should Engineers and Scientists Be Worried About Color?" [(pdf)][worry-about-color] by Bernice E. Rogowitz and Lloyd A. Treinish of IBM Research , h/t [\@EdwardTufte](https://twitter.com/EdwardTufte).
