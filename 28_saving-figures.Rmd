...
<!--[Link to original content](https://stat545.com/block017_write-figure-to-file.html)-->
...
<!--Removed img/evil-mouse.jpg because I couldn't find the original source-->
...
* __Machine-readability__: Every modern OS provide a way to search your hard drive for a file with a specific name or containing a specific string. This will be easier if the name contains no spaces, punctuation, or other funny stuff. If you use conventional extensions, you can even narrow the search to files ending in `.R` or `.Rmd`.
...
<!--
The original stat545 lesson had all of the (visible) code in one big chunk and the four plots together in one square. This was my attempt at breaking the code and the plots into two separate figures and weaving it into the text so that it better matches the structure in the text itself.
-->
...
```{r exaggerated-scale, echo = FALSE, fig.cap = "Two versions of a figure with exaggerated values of `scale`", fig.show='hold', out.width='50%'}
p <- ggplot(gapminder, aes(x = year, y = lifeExp)) + geom_jitter()
...
```{r exaggerated-base-size, echo = FALSE, fig.cap = "Two versions of a figure with exaggerated values of `base_size`", fig.show='hold', out.width='50%'}
p <- ggplot(gapminder, aes(x = year, y = lifeExp)) + geom_jitter()

p + ggtitle("base_size = 20") + theme_grey(base_size = 20)
p + ggtitle("base_size = 3") + theme_grey(base_size = 3)
```
