...
<!--[Link to original content](https://stat545.com/packages01_system-prep.html)-->
...
Embarking on your career as an R package developer is an important milestone. Why not celebrate by updating R and RStudio? This is something we recommended early and we recommend doing it often. [Go back to Day 0 of the course](#install) for reminders on the process. **DO IT NOW. We are not very interested in solving problems that stem from running outdated versions of R and RStudio.**
...
*We've never had this section but [RStudio's `devtools` guide][rstudio-devtools] and [R Packages][r-pkgs-intro-get] both say the `r-devel` or `r-base-dev` package is required. What gives?*
