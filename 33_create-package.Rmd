...
<!--[Link to original content](http://stat545.com/packages06_foofactors-package.html)-->


This section closely tracks with content from the book [R Packages](https://r-pkgs.org/) by Hadley Wickham and Jenny Bryan, specifically the chapter ["The Whole Game."](https://r-pkgs.org/whole-game.html) The material in this chapter is, however, accurate and up-to-date, although a bit longer than ["The Whole Game."](https://r-pkgs.org/whole-game.html) The [R Packages](https://r-pkgs.org/) book is a great place to go next to learn more. 


```{r, include = FALSE}
source("common.R")
knitr::opts_chunk$set(cache = FALSE, error = TRUE, warning = FALSE, message = FALSE)
...
We use the `devtools` package to facilitate package development. It’s on [CRAN](https://cloud.r-project.org/web/packages/usethis/index.html) and developed on [GitHub](https://devtools.r-lib.org/). Why devtools? 
...
The helper `use_devtools()` creates this file, if needed, opens it for editing, and puts the necessary lines of code on the clipboard and the screen. Another package you may want to handle this way is `testthat`.
...
You may notice that some functions we'll use in this chapter start with `use_*()`. These functions derive from the [`usethis` package](https://usethis.r-lib.org/), which is also on [CRAN](https://cloud.r-project.org/web/packages/usethis/index.html) and developed on [GitHub](https://github.com/r-lib/usethis). Generally, you should not need to worry about these different packages, because `devtools` installs them all automatically for you. `devtools` remains the primary package developers will interact with when writing R packages; it just relies on these other packages internally for most of the functionality. 
...
The `usethis::create_package()` function initializes a new package in a new directory on your computer. The new package will comply with all devtools conventions and will immediately pass `R CMD check`. Ideally, your package will always be in this happy state and it is something you should verify often.
...
What files are in this new directory? Here's a listing using the [fs package](https://fs.r-lib.org/) (locally, you can consult your file browser):
...
```
  ...
  * `DESCRIPTION` provides [metadata about your package](https://r-pkgs.org/description.html). We edit this shortly.
...
The foofactors directory is an R source package and an RStudio Project. Now we make it also a Git repository, with `usethis::use_git()`.
...
Huh? Many people do not expect the result of combining two factors to be an integer vector consisting of the numbers 1, 2, 3, and 4.
...
How do we move our local source package through the necessary stages to get it properly installed? (Figure from [R Packages](https://r-pkgs.org/package-structure.html#package).)
...
```{r first-check-no-doc, eval = create, warning = TRUE, echo = FALSE}
shhh_check(document = FALSE, error_on = "never")
```
...
At this point, we expect 2 warnings (and 0 errors, 0 notes):
  ...
  * `Non-standard license specification: What license is it under?`
...
Before we tackle the warnings about the license and documentation, we replace some of the boilerplate content in `DESCRIPTION`. The `DESCRIPTION` file provides metadata about your package:
  ...
  * read more from [R Packages](https://r-pkgs.org/description.html)
...
Make these edits:
  ...
  * Write some descriptive text in the `Title` and `Description` fields. Note that CRAN is very particular about these fields, so if you want to keep passing `check()`, read [this section](https://r-pkgs.org/description.html#pkg-description) of R Packages.
...
```{r commit-license, echo = debug, comment = NA}
...
For future projects, there is more guidance on licenses in these sources:
  ...
  * [R Packages](https://r-pkgs.org/description.html#license)
...
First, we declare our intent to write unit tests and to use the testthat package for this, via `usethis::use_testthat()`:
...
For much more guidance, read the [testing chapter](https://r-pkgs.org/tests.html) in R Packages.
...
There is more than one way to go about this. We use the approach recommended in the [namespace chapter](https://r-pkgs.org/namespace.html) of R Packages and in the [rOpenSci Packaging Guide](https://ropensci.github.io/dev_guide/building.html#package-dependencies).  The general usage pattern is this: give the name of the external package, then two colons, then the function to call.
...
Create a new file `R/fcount.R` with this function definition:
...
Let's start adding package-level documentation. Your package as a whole can have its own `.Rd` file. You can see an example by entering `package?dplyr` in R Console. Read more about the uses of package-level documentation in [R Packages](https://r-pkgs.org/man.html#man-packages).
...
A piece of package-level documentation that's probably familiar to you is the vignette. This is a great place to put a fully developed example that calls multiple functions from your package to do something useful and realistic. See the [vignettes chapter](https://r-pkgs.org/vignettes.html) of R Packages for more.
...
Here's what happens:
  ...
  * If using RStudio, open `vignettes/foofactors-vignette.Rmd` for editing.
...
The vignette source can be seen here: [foofactors-vignette.Rmd](https://raw.githubusercontent.com/apreshill/stat545-reboot/master/supporting-docs/foofactors-vignette.Rmd)
...
Now that your package is on GitHub, the `README.md` file matters. It is the package's home page and welcome mat, at least until you decide to give it a website (see [pkgdown](https://pkgdown.r-lib.org)) or submit it to CRAN (see the [Releasing a package chapter](https://r-pkgs.org/release.html) in the R Packages book).
...
```{r commit-readme-rmd, echo = debug, comment = NA}
paths <- unlist(status(repo))
add(repo, path = paths)
...
```{r copy-readme-rmd, include = debug}
...
```{r commit-rendered-readme, echo = debug, comment = NA}
paths <- unlist(status(repo))
add(repo, path = paths)
commit(repo, message = "Write README.Rmd and render")
```

```{r final-push, eval = github, include = debug}
