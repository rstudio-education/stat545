...
## Why `devtools` and `usethis`?

We use the `devtools` and `usethis` packages to facilitate package development. It's on [CRAN](https://cran.r-project.org/web/packages/devtools/index.html) and developed on [GitHub](https://github.com/hadley/devtools). Why devtools?
...
Create a new package in a new directory with `create_package()`:
...
use_mit_license("Jennifer Bryan")
```
...
vignette_path <- path("vignettes", "hello-foofactors.Rmd")
