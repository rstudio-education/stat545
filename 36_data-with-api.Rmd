...
In the simplest case, the data that you want is already available on the internet in a familiar format such as a csv and you can use `readr::read_csv()` to load it directly into R (see Chapter \@ref(import-export)).
...
If you have never heard of an API, we recommend this [introduction to APIs from Zapier](https://zapier.com/learn/apis/).
...
An API allows you to programmatically get resources (e.g. a csv file, html file, etc.) from a website. It is essentially facilitating a conversation: you send the web server a request and then the server sends you back a response. This response can either be the data you asked for or an error message.
