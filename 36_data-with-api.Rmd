...
![Image Source: https://medium.com/@perrysetgo/what-exactly-is-an-api-69f36968a41f](img/api-diagram.png)
...
Many popular API's have helper R packages are "wrappers" around the original API. These packages generally will have functions that assist with formatting a query, sending the query to the web server, and finally parsing the response sent back.
...
Here are a few popular API wrapper packages for API's that you may have heard of (see _resources_ for more):
...
The record that we will explore is the ["Headcount of Homeschool Children"](https://catalogue.data.gov.bc.ca/dataset/headcount-of-homeschooled-children),which contains data about the number of homeschool children registered between 1991 and 2018. Our end goal is to visualize how the headcounts varies between male and female students over the school years. The first step is to get the data into R. We can use `bcdata` to help us with this.

Install the package from GitHub (only do once) and then load it.
...
The `bcdata` [website](https://bcgov.github.io/bcdata/) gives us a nice overview of the main functions in the package and links to a ["Get started" vignette](https://bcgov.github.io/bcdata/articles/bcdata.html) available. This vignette is a good starting point to get familiar with this package. Let's follow along with the vignette to get the data that we're looking for.

First, we will use the [`bcdc_browse()`](https://bcgov.github.io/bcdata/reference/bcdc_browse.html) function to open up the B.C. Data Catalogue in our browser and find the record that we want.
...
On this page, under the Data and Resources section, we can see that there are six files available. Let's figure out how to get the first csv into R. 
...
Where do we get the resource ID from? It isn't quite clear in the `bcdc_get_data()` documentation so we might need to explore a bit. Let's start by looking at the `bcdc_get_record()` function that was mentioned earlier. 
