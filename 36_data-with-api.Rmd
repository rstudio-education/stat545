...
Using the package [`tidycensus`](https://github.com/walkerke/tidycensus)
```{r warning = FALSE}
...
`tidycensus` provides a utility function `census_api_key` that will put your API key into your .Renviron file but let's do this ourselves using `usethis`
...
Example
```{r}
get_estimates(geography = "state", product = "components", key = Sys.getenv("CENSUS_API_KEY")) %>% 
  head() %>% 
  kable()
```
