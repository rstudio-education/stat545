...
Here's what we get when we enter `title = "the Martian"` and `year = 2015`
...
Try pasting this link into the browser. Also experiment with changing `response` from `JSON` to `XML`.

How can we create the same request in R?
...
A similar process exists for XML formats using `XML::xmlParse()`:
...
```{r eval = FALSE}
the_martian <- GET("http://www.omdbapi.com/?", query = list(t = "The Martian", 
                                                            y = 2015, 
                                                            plot = "short", 
                                                            r = "json"))
content(the_martian)
