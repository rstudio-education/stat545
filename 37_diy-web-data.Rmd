...
1. __Add `.Rprofile` to your `.gitignore` !!__
1. Make a `.Rprofile` file ([windows tips](http://cran.r-project.org/bin/windows/rw-FAQ.html#What-are-HOME-and-working-directories_003f); [mac tips](http://cran.r-project.org/bin/macosx/RMacOSX-FAQ.html#The-R-Console)).
1. Write the following in it:

    ```{r eval = FALSE}
    options(OMBD_API_KEY = "YOUR_KEY")
    ```

1. Restart R (i.e. reopen your RStudio project).
...
Our JSON response above can be parsed with the [jsonlite] package. First install/load the jsonlite package.
...
Parsing our JSON response with `jsonlite::fromJSON()`:
...
Parsing our XML response with `xml2::read_xml()`:
...
The [httr] package is yet another star in the tidyverse. It is designed to facilitate all things HTTP from within R. This includes the major HTTP verbs, which are:
...
1. The [rvest] package. Install via `install.packages("rvest)"`.
