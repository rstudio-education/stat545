...
In Chapter \@ref(api-wrappers) we experimented with several packages that "wrapped" APIs.  That is, they handled the creation of the request and the formatting of the output. Today we're going to look at (part of) what these functions were doing.
...
```{r web-data-omdb, echo = FALSE, fig.cap = "Example OMDb Query"}
library(knitr)
knitr::include_graphics("img/ombd.png")
```
...
It works, but it's a bit ungainly.Let's try to abstract that into a function:
...
<!--TODO: The following code chunks are broken; OMDb requires an API key now-->
```{r message = FALSE, warning = FALSE}
library(curl)
```
...
From [XML and Web Technologies for Data Sciences with R](https://link.springer.com/book/10.1007%2F978-1-4614-7900-0) by Nolan & Lang [-@nolan2013]
...
<!--TODO: It's not clear in the original stat545 lesson what this is referring to-->
...
We have created a simple html table with the head of gapminder in it! We can get our data back by parsing this table into a data frame again. Extracting data from html is called "scraping", and it is done with the R package `rvest`:
...
If you look at the structure of `research` (i.e. with `str(research)`) you'll see that we've obtained a list of data.frames. The top of the page contains another table element. This was also scraped! 
