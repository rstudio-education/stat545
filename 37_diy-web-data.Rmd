...
Here's what we see for `title = Interstellar`, `year = 2014`, `plot = full`, and `response = JSON`:
...
This tells us that we need an API key to access the OMDb API. We used an API key to access the PLOS ONE API in the [previous chapter](#plos-one) and stored the key in our `.Renviron` file using `Sys.setenv()`. We will store our key for the OMDb API in our `.Renviron` file again, but this time do so by using the helper function `edit_r_environ()` from the [`usethis`][usethis-web] package. Follow these steps:
...
4. Add `OMDB_API_KEY=<your-secret-key>` on a new line, press enter to add a blank line at the end (important!), save the file, and close it.  
5. Restart R.

You can now access your OMDb API key with `Sys.getenv("OMDB_API_KEY")`. We can use this to easily add our API key to the request URL.

How can we recreate the same request URLs in R? We could use `str_c()` from the `stringr` package to paste together the base URL, parameter labels, and parameter values:

```{r}
request <- str_c("http://www.omdbapi.com/?",
                 "t=", "Interstellar", "&", 
                 "y=", "2014", "&", 
                 "plot=", "short", "&", 
                 "r=", "xml", "&",
...
This works, but it's a bit ungainly. Let's try to abstract that into a function (see Chapters \@ref(functions-part1), \@ref(functions-part2), \@ref(functions-part3), and \@ref(functions-practicum) for a review on writing functions in R):

```{r}
omdb <- function(title, year, plot, format, api_key){
  baseurl <- "http://www.omdbapi.com/?"
  params <- c("apikey=", "t=", "y=", "plot=", "r=")
  values <- c(api_key, title, year, plot, format)
  param_values <- map2(params, values, str_c)
  args <- str_c(param_values, collapse = "&")
  str_c(baseurl, args)
}

omdb(title = "Interstellar", year = "2014", plot = "short", format = "xml", 
     api_key = "mysecretkey")
```
...
There are easy means of taking these data types and creating R objects. 
...
```{r echo = FALSE, fig.cap = "A simple HTML table", out.width = "100%"}
knitr::include_graphics("img/gapminderhead.png")
```
...
**MOST IMPORTANTLY** confirm that there is NO rOpenSci package and NO API before you [spend hours scraping](https://rpubs.com/aammd/kivascrape) (the [API was right here](http://build.kiva.org/)).
